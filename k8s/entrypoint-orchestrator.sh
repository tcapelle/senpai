...
2. Read agent status files: ls /mnt/new-pvc/senpai/status/ — each agent writes a JSON with its current state, metrics, and what it's working on
3. Query W&B (capecape/senpai) for detailed metrics and run history
...
6. Write instructions for agents: write JSON to /mnt/new-pvc/senpai/orchestrator/instructions.json — agents check this before starting new experiments

Instructions file format:
{
  "sota": {"val_loss": 12.3, "branch": "exp/wider-model", "agent": "pepe"},
  "avoid": ["L1 loss (worse)", "batch_size > 8 (OOM)"],
  "suggestions": {"agata": "try multi-scale attention", "claudio": "explore gradient-based loss"}
}
...
- Status files: /mnt/new-pvc/senpai/status/<agent>.json (read), /mnt/new-pvc/senpai/orchestrator/ (write)
...
Start by surveying the current state: read all agent status files, check what agents are running, query W&B for the current best metrics. Then wait for instructions.
EOF
